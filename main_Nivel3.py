...
sprite_sheet = pygame.image.load(os.path.join(IMAGEM_PATH, 'sequencia_Nivel3.png')).convert_alpha()
...
class Corvos(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = sprite_sheet.subsurface((5*25, 0), (25,23))
        self.image = pygame.transform.scale(self.image, (25*3, 23*3))
        self.rect = self.image.get_rect()
...
class Fogo(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = sprite_sheet.subsurface((6*25, 0), (25,23))
        self.image = pygame.transform.scale(self.image, (25*2, 23*2))
        self.rect = self.image.get_rect()
        self.mask = pygame.mask.from_surface(self.image)
        self.escolha = escolher_obstaculo
...
for i in range(0):#deixar zero
    corvo = Corvos()
    tudo_sprites.add(corvo)
...
while True:
    ...
    if fogo.rect.topright[0] <= 0 or fox_pulador.rect.topright[0] <= 0:
        escolher_obstaculo = choice([0, 1])
        fogo.rect.x = WIDTH
        fox_pulador.rect.x = WIDTH
        fogo.escolha = escolher_obstaculo
        fox_pulador.escolha = escolher_obstaculo
